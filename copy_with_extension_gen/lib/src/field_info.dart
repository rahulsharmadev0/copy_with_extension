...
import 'package:analyzer/dart/element/element.dart' show ClassElement, FieldElement, ParameterElement;
...
class FieldInfo {
  FieldInfo({required this.name, required this.nullable, required this.type, required this.isOptional});
...
class ConstructorParameterInfo extends FieldInfo {
  ...
  static CopyWithFieldAnnotation _readFieldAnnotation(
    ...
    final removable = reader.peek('removable')?.boolValue;

    return CopyWithFieldAnnotation(
      immutable: immutable ?? defaults.immutable,
      removable: removable ?? defaults.removable,
    );
  }
}
