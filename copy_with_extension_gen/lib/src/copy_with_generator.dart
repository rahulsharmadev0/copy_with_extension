...
class CopyWithGenerator extends GeneratorForAnnotation<CopyWith> {
  ...
  String generateForAnnotatedElement(
    ...
    final sortedFields = sortedConstructorFields(classElement, classAnnotation.constructor);
    ...
    for (final field in sortedFields) {
      if (field.classFieldInfo != null && field.nullable == false && field.classFieldInfo?.nullable == true) {
        throw InvalidGenerationSourceError(
  ...
  String _copyWithProxyPart(
    ...
    final filteredFields = sortedFields.where((e) => !e.fieldAnnotation.immutable);
    ...
    ''').join("\n");
    final nonNullableFunctionsInterface = skipFields ? "" : filteredFields.map((e) => '''
  ...
  String _copyWithValuesPart(
    ...
    final constructorInput = sortedFields.fold<String>(
      ...
      (r, v) {
        ...
        if (isAbstract) {
          final type = v.type.endsWith('?') || v.isDynamic ? v.type : '${v.type}?';
    ...
    final paramsInput = sortedFields.fold<String>(
      '',
      (r, v) {
        if (v.fieldAnnotation.immutable && v.fieldAnnotation.removable && v.isOptional) return r;

        if (v.fieldAnnotation.immutable) return '$r ${v.isPositioned ? "" : '${v.name}:'} _value.${v.name},';
    ...
    final constructorBody =
        isAbstract ? "" : "{ return ${constructorFor(typeAnnotation, constructor)}($paramsInput); }";
